...
Le bouton `effect_` ouvre quatre choix `fxChoice_` en capsules, sans changer la valeur. Le choix applique directement le mode et le séquenceur reste indépendant. Pour éviter qu’un contrôle réapparaisse sous le pointeur pendant le même événement ControlP5, l’ouverture/fermeture du menu et le `layoutInterface` associé sont différés jusqu’au relâchement de la souris via `pendingEffectMenuChannel`, `layoutPending` et `applyPendingLayout`. `refreshPortButtons` construit des pages de six boutons de ports à partir de `availablePorts` ; `refreshPorts` actualise cette liste par `Serial.list()`.
