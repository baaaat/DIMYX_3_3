...
final int NO_PENDING_EFFECT_MENU = -2;
int pendingEffectMenuChannel = NO_PENDING_EFFECT_MENU;
...
void requestEffectMenu(int target) {
  pendingEffectMenuChannel = target;
  layoutPending = true;
}

void applyPendingLayout() {
  // Ne jamais masquer ou deplacer un controle ControlP5 pendant un clic.
  if (!layoutPending || mousePressed) return;

  if (pendingEffectMenuChannel != NO_PENDING_EFFECT_MENU) {
    effectMenuChannel = pendingEffectMenuChannel;
    pendingEffectMenuChannel = NO_PENDING_EFFECT_MENU;
  }
...
void mousePressed() {
  if (effectMenuChannel >= 0) {
    ...
    boolean outside = mouseX < x || mouseX >= x + stripControlWidth || mouseY < 120 || mouseY >= 280;
    if (outside) {
      requestEffectMenu(-1);
      // Ce clic sert uniquement a fermer le menu : il ne doit pas atteindre un controle dessous.
      return;
    }
  }
  if (outputsView || (narrowLayout && scenesView)) return;
...
public void controlEvent(ControlEvent e) {
  ...
  if (e.isController() && e.getName().startsWith("effect_")) {
    ...
    int currentMenu = pendingEffectMenuChannel != NO_PENDING_EFFECT_MENU ? pendingEffectMenuChannel : effectMenuChannel;
    requestEffectMenu(currentMenu == i ? -1 : i);
    return;
  }
  if (e.isController() && e.getName().startsWith("fxChoice_")) {
    ...
    updateEffectButton(i);
    // Le menu reste physiquement en place jusqu'au relachement de la souris.
    requestEffectMenu(-1);
    return;
  }
